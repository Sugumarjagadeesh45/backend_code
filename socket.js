...
async function generateSequentialRaidId() {
  try {
    ...
    if (sequenceNumber > 999999) {
      ...
      await RaidId.findOneAndUpdate({ _id: "raidId" }, { sequence: 100000 });
  ...
  io.on("connection", (socket) => {
    ...
    socket.on("userLocationUpdate", async (data) => {
      try {
        ...
        console.log(`📍 USER LOCATION UPDATE: User ${userId}`);
        console.log(`🗺️ Coordinates: ${latitude}, ${longitude}`);

        // Verify ride exists and is active
        if (rideId && rides[rideId]) {
          const rideStatus = rides[rideId].status;
          if (!["pending", "accepted", "started"].includes(rideStatus)) {
            console.log(`🚫 Ignoring user location update: Ride ${rideId} is ${rideStatus}`);
            return;
          }

          // Store user location in tracking map
          ...
          // Send only to the assigned driver
          if (rides[rideId].driverId) {
            const assignedDriverId = rides[rideId].driverId;
            const driverRoom = `driver_${assignedDriverId}`;

            io.to(driverRoom).emit("userLiveLocationUpdate", {
              rideId,
              userId,
              lat: latitude,
              lng: longitude,
              driverId: assignedDriverId, // Added to validate on frontend
              timestamp: Date.now(),
            });

            console.log(`📡 Sent user location to ASSIGNED driver ${assignedDriverId} in room ${driverRoom}`);
          } else {
            console.log(`ℹ️ No assigned driver for user ${userId}, not broadcasting location`);
          }
        } else {
          console.log(`🚫 Ignoring user location update: No valid ride found for ${rideId}`);
        }
      } catch (error) {
    ...
    socket.on("bookRide", async (data, callback) => {
      ...
      try {
        ...
        // CRITICAL FIX: Only send to drivers who are NOT on a ride
        const availableDrivers = Array.from(activeDriverSockets.values()).filter(
          (driver) => driver.isOnline && driver.status !== "onRide"
        );
        ...
        console.log(`📡 Ride request sent to ${availableDrivers.length} available drivers`);
        ...
        console.log(`📡 Ride request broadcasted to available drivers with ID: ${rideId}`);
        console.log(`💾 Ride saved to MongoDB with ID: ${savedRide._id}`);
    ...
    socket.on("acceptRide", async (data, callback) => {
      ...
      try {
        ...
        // Send only to the specific driver's room
        io.to(`driver_${driverId}`).emit("userDataForDriver", userDataForDriver);
        console.log(`✅ User data sent to driver room: driver_${driverId}`);
    ...
    socket.on("cancelRide", async (data) => {
      try {
        ...
        if (rides[rideId]) {
          ...
          rides[rideId].cancelledAt = Date.now();
          ...
          if (rides[rideId].driverId) {
            ...
            if (activeDriverSockets.has(driverId)) {
              ...
              io.to(`driver_${driverId}`).emit("driverStatusUpdate", {
                driverId,
                status: "Live",
              });
          ...
          if (userLocationTracking.has(userId)) {
            userLocationTracking.delete(userId);
            console.log(`🗑️ Removed user location tracking for user: ${userId}`);
          }

          // Remove ride after 5 seconds
          setTimeout(() => {
            delete rides[rideId];
            console.log(`🗑️ Removed cancelled ride: ${rideId}`);
          }, 5000);
    ...
    socket.on("completeRide", async (data) => {
      try {
        ...
        if (rides[rideId]) {
          ...
          // CRITICAL FIX: Update driver status back to online
          if (activeDriverSockets.has(driverId)) {
            const driverData = activeDriverSockets.get(driverId);
            driverData.status = "Live";
            driverData.isOnline = true;
...
//         // If user has an active ride, update the ride data
//         if (rideId && rides[rideId]) {
//           rides[rideId].userLocation = { latitude, longitude };
//           console.log(`✅ Updated user location in ride ${rideId}`);
//         }
        
//         // Broadcast user location to all drivers
//         io.emit("userLiveLocationUpdate", {
//           userId,
//           lat: latitude,
//           lng: longitude,
//           rideId: rideId || null,
...
//         console.log(`📡 Broadcasted user ${userId} location to all drivers`);
...
//           console.log(`❌ No driver assigned for ride ${rideId}`);
//           return;
//         }
...
//         console.log(`📡 Sent user location to driver ${ride.driverId} in room ${driverRoom}`);
